...
/*            foreach (Vent vent in MapOptions.ventsToSeal)
    ...
    class ExileControllerWrapUpPatch
    {
        ...
        static void WrapUpPostfix(GameData.PlayerInfo exiled)
        {
            ...
            // Seer spawn souls
            if (Seer.deadBodyPositions != null && Seer.seer != null && PlayerControl.LocalPlayer.isRole(CustomRoleTypes.Seer) && (Seer.mode == 0 || Seer.mode == 2))
            {
                foreach (Vector3 pos in Seer.deadBodyPositions)
                {
                    ...
                    {
                        DestroyableSingleton<HudManager>.Instance.StartCoroutine(Effects.Lerp(Seer.soulDuration, new Action<float>((p) =>
                        {
                            if (rend != null)
                            {
                                var tmp = rend.color;
        ...
        static void Postfix(ref string __result, [HarmonyArgument(0)] StringNames id)
        {
            try
            {
                if (ExileController.Instance != null && ExileController.Instance.exiled != null)
                {
                    ...
                    {
                        if (player.isRole(CustomRoleTypes.Jester)) __result = "";
                    }
                }
            }
            catch
